...
Gateway 是后端唯一的对外实时通信入口，合并了原来 hermes-webui 的 `/ws/caicai`（WebSocket）和 `/api/game/events`（SSE）两条通道。
...
| GET | `/api/game/rooms` | 获取所有房间列表 |
| GET | `/api/game/agents` | 获取所有 Agent |
| GET | `/api/game/tasks` | 获取所有任务 |
| POST | `/api/game/agent` | 添加新 Agent |
| POST | `/api/game/agent/update` | 更新 Agent 属性 |
| POST | `/api/game/agent/move` | 移动 Agent 到房间 |
| POST | `/api/game/task` | 创建任务 |
| POST | `/api/game/task/assign` | 分配任务（含竞争判定） |
| POST | `/api/game/greeting` | 触发打招呼 |
| POST | `/api/game/collaboration` | 触发协作任务 |
| GET | `/api/game/competition/history` | 获取竞争历史记录 |
