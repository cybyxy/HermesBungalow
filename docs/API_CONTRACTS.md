...
> 与实现对照：Python 入口 [backend/server.py](../backend/server.py)（Starlette 路由表，52 条路由），游戏域 [backend/api/task/](../backend/api/task/)。
> 基线端口：`http://127.0.0.1:8765`（与 [frontend/vite.config.ts](../frontend/vite.config.ts) 代理一致）。
>
> **文档状态**：v1.2 — 新增任务链 DAG、城主入口、消息回执、Agent 社交、任务更新 API
...
```
职责：
├── 游戏状态管理（TaskWorld SQLite 持久化）
├── REST API（/api/task/*）— Agent/任务/房间/存档 CRUD
├── WebSocket Gateway（/ws/gateway）— 广播游戏事件到前端
├── LLM 事件标签解析（/api/task/llm/apply-tags）— 解析 [[GAME_EVENT:...]]
├── 游戏 Tick 推进（/api/task/tick）
...
|------|------|------|----------|
| GET | `/health` | 服务健康 | 有 |
| GET | `/api/task/state` | 完整 `TaskWorld` JSON | 有 |
| GET | `/api/task/agents` | Agent 列表 | 有 |
| GET | `/api/task/tasks` | 任务列表 | 有 |
| GET | `/api/task/rooms` | 房间列表 | 有 |
| POST | `/api/task/agent` | 添加 Agent（body JSON） | 有 |
| POST | `/api/task/agent/update` | 更新 Agent 字段 | 有 |
| POST | `/api/task/agent/move` | `{ agent_id, room_id }` | 有 |
| POST | `/api/task/task` | 创建任务 | 有 |
| POST | `/api/task/task/assign` | 分配任务（含简单竞争） | 有 |
| POST | `/api/task/greeting` | 触发打招呼（占位 + 事件） | 有 |
| POST | `/api/task/collaboration` | 协作占位 + 事件 | 有 |
| GET | `/api/task/competition/history` | 竞争历史 | 有 |
| GET | `/api/task/save` | 读取存档元数据 / 快照 | 有 |
| PUT | `/api/task/save` | 写入存档快照 | 有 |
| POST | `/api/task/llm/apply-tags` | 解析 LLM 文本中的游戏事件标签并应用 | 有 |
| POST | `/api/task/tick` | 推进游戏内时间（body 可选 `{ "minutes": N }`，默认 1 分钟/次） | 有 |
| POST | `/api/task/task/complete` | 完成任务（`{ task_id, quality }`），完成后触发锁状态重算 | 有 |
| POST | `/api/task/task/delete` | 删除任务，清理依赖引用 + 重算锁状态 | 有 |
| POST | `/api/task/task/update` | 更新任务字段（name, description, status, depends_on 等） | v1.2 |
| POST | `/api/task/task/dependency` | 设置任务依赖 `{ task_id, depends_on }` | v1.2 |
| POST | `/api/task/task/batch-create` | 批量创建任务（含批内依赖索引解析） | v1.2 |
| GET | `/api/task/task-board/query` | 任务板查询（可按 status / profession 过滤） | v1.2 |
| POST | `/api/task/task/claim` | 认领任务（仅 unlocked + pending） | v1.2 |
| GET | `/api/task/task-chain` | 任务 DAG 可视化数据（nodes + edges） | v1.2 |
| POST | `/api/task/lord/chat` | 城主入口（自动注入任务上下文 + 多轮协作） | v1.2 |
| POST | `/api/task/agent/social-chat` | Agent 空闲社交聊天 | v1.2 |
| POST | `/api/task/agent-relay` | 转发聊天到指定 Agent（@handoff） | v1.2 |
...
> - 与游戏 API 同端口共存：`/api/task/*` 保留在 Starlette 路由层，Hermes 通用接口经 `/api/*` 桥接进入搬迁模块。
...
以下类型建议在后续版本中与 `POST /api/task/*` 或 Gateway 合并；当前以前端 `services/gameApi.ts`（API 导出枢纽）的 HTTP 为主，Gateway 用于 **推送**。

| 方向 | type | 用途 |
|------|------|------|
| C→S | `assign_task` | 同 `POST /api/task/task/assign` |
| C→S | `move_agent` | 同 `POST /api/task/agent/move` |
...
支持 `type`：`task_progress` | `agent_mood` | `agent_move` | `money_delta` | `log` | `task_chain_create` | `artifact_create` | `task_workflow_plan`（详见 [backend/api/task/llm_events.py](../backend/api/task/llm_events.py)）。

**v1.2 新增事件**：
| type | 说明 | 关键字段 |
|------|------|----------|
| `task_chain_create` | 自然语言创建项目任务链 | `name`, `description`, `sub_tasks[]`（含 `depends_on_indices`） |
| `artifact_create` | 创建产出物（PRD/设计文档等） | `kind`, `title`, `content`, `task_id` |
| `task_workflow_plan` | 任务流程规划 | `task_id`, `steps[]` |

## 五、游戏编排 SSE（`server.py` 内联 Hermes）

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/task/agent-chat-orchestrated/run` | Body 同 `POST /api/task/agent-chat-orchestrated`，返回 `{ ok, run_id, work_order_id }`，后台线程跑编排。 |
| GET | `/api/task/agent-chat-orchestrated/stream?run_id=` | `text/event-stream`，`data:` 行为 JSON；`type` 含 `step_begin`、`reasoning_delta`、`tool_start`、`tool_end`、`assistant_message`、`step_done`、`delegation_start`、`error`、`stopped`、`turn_done`。 |
| POST | `/api/task/agent-stream/cancel` | Body `{ "stream_id" }`，转发 Hermes `cancel_stream`。 |
