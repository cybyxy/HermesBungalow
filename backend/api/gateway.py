...
import subprocess
import re
from ..services.hermes_cli_api import HermesCliApi

router = APIRouter(prefix="/api", tags=["Gateway"])
ENERGY_STATE_FILE = Path(os.path.expanduser("~/.hermes/hermes-bungalow-energy.json"))
HERMES_WORKSPACE = "/Users/bobo/ai_projects/HermesBungalow"
hermes_cli = HermesCliApi(workspace=HERMES_WORKSPACE)

# WebSocket连接管理
active_connections: list[WebSocket] = []
LATEST_SESSION_BOOTSTRAP: dict = {"session_id": "", "messages": []}


def _hermes_run(args: list[str], timeout: int = 12):
    return hermes_cli.run_sync(args, timeout=timeout, check=False)


def _extract_session_messages_from_export_text(session_id: str, export_text: str) -> list[dict]:
    ...
    def _collect_msgs(obj, out: list[dict]) -> None:
    ...
    for raw in (export_text or "").splitlines():
        ...
        try:
            ...
            continue
        _collect_msgs(payload, collected)

    normalized = []
    for idx, msg in enumerate(collected):
        normalized.append({
            "id": f"{session_id}-{idx}",
            "sender": "user" if msg["role"] == "user" else "caicai",
            "text": msg["text"],
...
def _load_session_history(session_id: str) -> dict:
    ...
    proc = _hermes_run(["sessions", "export", "-", "--session-id", session_id], timeout=20)
    if proc.returncode != 0:
        return {"session_id": session_id, "messages": [], "error": (proc.stderr or proc.stdout or "").strip()}
    return {
        "session_id": session_id,
        "messages": _extract_session_messages_from_export_text(session_id, proc.stdout),
    }


def refresh_latest_session_bootstrap() -> dict:
    global LATEST_SESSION_BOOTSTRAP
    res = _hermes_run(["sessions", "list"], timeout=12)
    if res.returncode != 0:
        LATEST_SESSION_BOOTSTRAP = {"session_id": "", "messages": []}
        return LATEST_SESSION_BOOTSTRAP
    latest_sid = ""
    for raw in (res.stdout or "").splitlines():
        ...
        if m:
            latest_sid = m.group(1)
            break
    if not latest_sid:
        LATEST_SESSION_BOOTSTRAP = {"session_id": "", "messages": []}
        return LATEST_SESSION_BOOTSTRAP
    LATEST_SESSION_BOOTSTRAP = _load_session_history(latest_sid)
    return LATEST_SESSION_BOOTSTRAP
...
@router.post("/v1/chat")
async def chat_v1_stream(payload: dict):
    """
    兼容 Hermes Gateway 风格流式接口：
    POST /api/v1/chat
    """
    ...
    force_new_session = not bool(session_id)
    ...
    if not stream:
        ...
        async for chunk in chat_service.send_message(
            ...
            if chunk.get("type") == "token":
                full += str(chunk.get("text", ""))
            elif chunk.get("type") == "done":
                done = chunk
        return {
            "type": "done",
            "reply": full or done.get("reply", ""),
            "session_id": (done.get("session") or {}).get("session_id"),
...
async def get_hermes_commands():
    """
    获取 Hermes CLI 全部可用指令（含一级子命令）。
    """
    try:
        tree = hermes_cli.get_commands_tree()
        return {"ok": True, **tree}
    except Exception as e:
        return {"ok": False, "root": [], "subcommands": {}, "error": str(e)}


@router.get("/hermes/session-history")
async def get_hermes_session_history(session_id: str):
    """
    按 session_id 读取 Hermes 历史会话消息（用于前端切换会话后回显）。
    """
    try:
        return _load_session_history(session_id)
    except Exception as e:
        return {"session_id": (session_id or "").strip(), "messages": [], "error": str(e)}
...
async def list_hermes_skills():
    ...
    try:
        proc = _hermes_run(["skills", "list", "--enabled-only"], timeout=15)
...
async def get_hermes_model_config():
    ...
    config_path = ""
    try:
        cfg_path_proc = _hermes_run(["config", "path"], timeout=8)
...
async def get_hermes_current_model():
    ...
    if not model:
        try:
            path_proc = _hermes_run(["config", "path"], timeout=8)
...
async def get_chat_memory_check():
    ...
    try:
        cli_proc = _hermes_run(["--version"], timeout=8)
        ...
        status_proc = _hermes_run(["memory", "status"], timeout=8)
...
async def get_hermes_overview():
    ...
    try:
        version_proc = _hermes_run(["--version"], timeout=8)
        ...
        mem_proc = _hermes_run(["memory", "status"], timeout=8)
        ...
        skills_proc = _hermes_run(["skills", "list", "--enabled-only"], timeout=12)
...
async def run_hermes_health_check(include_chat: int = Query(0)):
    """
    ...
    if include_chat:
        ...
        # 后台静默轮询不跑 chat，避免每分钟写入 health-check 会话
        checks["chat"]["ok"] = True
        checks["chat"]["message"] = "skipped_in_background_poll"
    ...
    try:
        cfg_proc = _hermes_run(["config", "show"], timeout=12)
...
async def update_hermes_model_config(payload: dict):
    ...
    for field, cfg_key in field_to_key.items():
        ...
        if not value:
            continue
        try:
            proc = _hermes_run(["config", "set", cfg_key, value], timeout=12)
            if proc.returncode == 0:
...
async def ws_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        # 固定策略：连接成功后进入“新建空会话”，不自动加载最后历史会话内容
        current_sid = chat_service.get_current_session_id() or ""
        if current_sid:
            await websocket.send_json({
            ...
            async for chunk in chat_service.send_message(
                ...
                if chunk_type == "token":
                    ...
                    if resolved_session_id:
                        ws_session_id = str(resolved_session_id)
    ...
    try:
        while True:
            ...
            if msg_type == "chat":
                ...
                force_new_session = bool(msg.get("new_session", False))
