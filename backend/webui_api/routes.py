...
from .upload import handle_upload, handle_transcribe
from .streaming import _sse, _run_agent_streaming, cancel_stream
from .providers import get_providers, set_provider_key, remove_provider_key
...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path == "/api/auth/status":
        from .auth import is_auth_enabled, parse_cookie, verify_session
    ...
    if parsed.path == "/sw.js":
        ...
        if sw_path.exists():
            ...
            from .updates import WEBUI_VERSION
    ...
    if parsed.path == "/api/session/status":
        ...
        try:
            from .session_ops import session_status
    ...
    if parsed.path == "/api/session/usage":
        ...
        if not sid:
            return bad(handler, "Missing session_id")
        try:
            from .session_ops import session_usage
    ...
    if parsed.path == "/api/background/status":
        ...
        if not sid:
            return bad(handler, "Missing session_id")
        from .background import get_results
    ...
    if parsed.path == "/api/personalities":
        ...
        from .config import reload_config as _reload_cfg
    ...
    if parsed.path == "/api/git-info":
        ...
        try:
            ...
            return bad(handler, "Session not found", 404)
        from .workspace import git_info_for_workspace
    ...
    if parsed.path == "/api/profiles":
        from .profiles import list_profiles_api, get_active_profile_name
    ...
    if parsed.path == "/api/profile/active":
        from .profiles import get_active_profile_name, get_active_hermes_home
...
def handle_post(handler, parsed) -> bool:
    ...
    if parsed.path == "/api/personality/set":
        ...
        prompt = ""
        if name:
            from .config import reload_config as _reload_cfg2

            _reload_cfg2()  # pick up config changes without restart
            from .config import get_config as _get_cfg2
    ...
    if parsed.path == "/api/session/delete":
        ...
        from .config import _evict_session_agent
        ...
        try:
            from .models import delete_cli_session
    ...
    if parsed.path == "/api/session/retry":
        try:
            ...
            return bad(handler, str(e))
        try:
            from .session_ops import retry_last
    ...
    if parsed.path == "/api/session/undo":
        try:
            ...
            return bad(handler, str(e))
        try:
            from .session_ops import undo_last
    ...
    if parsed.path == "/api/chat/steer":
        from .streaming import _handle_chat_steer
    ...
    if parsed.path == "/api/profile/switch":
        ...
        try:
            from .profiles import switch_profile, _validate_profile_name
            from .helpers import build_profile_cookie
            if name != 'default':
    ...
    if parsed.path == "/api/profile/create":
        ...
        try:
            from .profiles import create_profile_api
    ...
    if parsed.path == "/api/profile/delete":
        ...
        if not name:
            return bad(handler, "name is required")
        try:
            from .profiles import delete_profile_api, _validate_profile_name
    ...
    if parsed.path == "/api/updates/force":
        ...
        if target not in ("webui", "agent"):
            return bad(handler, 'target must be "webui" or "agent"')
        from .updates import apply_force_update
    ...
    if parsed.path == "/api/auth/login":
        ...
        from .auth import _check_login_rate, _record_login_attempt
    ...
    if parsed.path == "/api/auth/logout":
        from .auth import clear_auth_cookie, invalidate_session, parse_cookie
...
def _handle_gateway_sse_stream(handler, parsed):
    ...
    from .gateway_watcher import get_watcher
    watcher = get_watcher()
    ...
    try:
        ...
        from .models import get_cli_sessions
...
def _handle_media(handler, parsed):
    ...
    try:
        from .workspace import get_last_workspace
...
def _handle_live_models(handler, parsed):
    ...
    - Copilot: live fetch from .githubcopilot.com/models with correct headers
    ...
    try:
        ...
        from .config import _resolve_provider_alias
        ...
        if not ids:
            from .config import _PROVIDER_MODELS as _pm
        ...
        from .config import _format_ollama_label as _fmt_ollama
...
def _handle_memory_read(handler):
    try:
        from .profiles import get_active_hermes_home
...
def _handle_btw(handler, body):
    ...
    from .models import new_session as _new_session
    ...
    with STREAMS_LOCK:
        STREAMS[stream_id] = q
    from .background import track_btw
...
def _handle_background(handler, body):
    ...
    from .background import track_background, complete_background
...
def _handle_chat_sync(handler, body):
    ...
    from .streaming import _ENV_LOCK
    ...
    try:
        ...
        with CHAT_LOCK:
            from .config import resolve_model_provider
            ...
            from .streaming import _sanitize_messages_for_api, _restore_reasoning_metadata
    ...
    try:
        if load_settings().get("sync_to_insights"):
            from .state_sync import sync_session_usage
...
def _handle_session_compress(handler, body):
    ...
    try:
        from .streaming import _sanitize_messages_for_api
